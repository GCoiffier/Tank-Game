...
# Note: All angles defined in these classes (Tank, Canon, Bullet...) are defined from the x axis and anti-clockwise.

class Body(pygame.sprite.Sprite):
    """Body: the body of a tank."""
    def __init__(self , tank_name,  pos):
        pygame.sprite.Sprite.__init__(self)
        self.image, self.rect = Utils.load_image(IMAGE_PATH, tank_name)
        ...
        self.rotation_step = math.radians(15)  #
    ...
    def update(self, walls_group=None, bliton = pygame.display.get_surface()):
        ...
        collided_walls = pygame.sprite.spritecollide(self, walls_group, False, pygame.sprite.collide_rect)
        if collided_walls:
...
class Canon(pygame.sprite.Sprite):
    ...
    def __init__(self, canon_name, pos, target_pos=None) :
        pygame.sprite.Sprite.__init__(self)
        self.image, self.rect = Utils.load_image(IMAGE_PATH, canon_name)
        self.imageBase = self.image
        self.rect.center = pos
    ...
    def update(self, pos, target_pos, bliton = pygame.display.get_surface()):
        self.rect.center = pos
        self._rotate(target_pos)
        bliton.blit(self.image, self.rect)
...
class Tank:
    ...
    def __init__(self, tank_name, canon_name , pos , target_pos=None) :
        self.body = Body(tank_name, pos)
        self.canon = Canon(canon_name, pos, target_pos)
    ...
    def create_bullet(self,target_pos):
        """
        Creates a new bullet going in target_pos' direction
        """
        ...
        created_bullet = Bullet_Cursor.Bullet(x_bullet, y_bullet, beta)
    ...
    def update(self, target_pos, bullets_group, walls_group=None, bliton = pygame.display.get_surface()):
        """
        updates the state of the tank, and displays it onto the screen.
        """
        destroyedSound = pygame.mixer.Sound(join(MUSIC_PATH, "destroyed_sound.wav"))
        ...
        if self.alive:
            for bullet in bullets_group:
                if pygame.sprite.collide_rect(bullet, self.body):
                    ...
                    self.alive = False
                    break
            self.body.update(walls_group, bliton)
            self.canon.update(self.body.rect.center, target_pos, bliton)
        else :
            bliton.blit(self.body.image, self.body.rect)
            bliton.blit(self.canon.image, self.canon.rect)
