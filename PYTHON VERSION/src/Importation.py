...
def get_level(path, n, custom=False): # n est le numéro du niveau à charger. -1 pour le menu. Si custom est True, on va chercher un niveau custom
    ...
    if n == -1 : # menu
        level_path = join(LEVEL_PATH, "menu.txt")
    elif n== -2 :# Training mode
        level_path = join(LEVEL_PATH, "training.txt")
    else:
        if custom:
            level_path = join(CLEVEL_PATH, "custom_level{}.txt".format(n))
        else:
            level_path = join(LEVEL_PATH, "level{}.txt".format(n))
        ...
        for i in range(21):
            for j in range(32):
                ...
                if var == 1:
                    ...
                    walls_group.add(Wall(typemur, i, j))
                elif var == 2:
                    typemur = get_type("pit",  get_voisins(map_niveau, i, j, 2))
                    pits_group.add(Pit(typemur, i, j ))
                else:
                    ...
                    elif var == 14: pos_AI.append(["advanced", pos])
                    elif var == 15: pos_AI.append(["advancedPlus",pos])
...
def init_unlocked(a_bool):
    """
    if a_bool : all level unlocked
    else : all level locked
    """
    unlock_path = join(MAIN_PATH, "unlocked.txt")
    with open(unlock_path,mode="w") as texte:
        n = 1
        while os.path.exists(join(LEVEL_PATH, "level{}.txt".format(n))):
            n += 1
        string = "1 1 "
        for i in range(1, n) :
